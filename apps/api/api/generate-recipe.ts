...
app.post('/', async (c) => {
  ...
  // Wrap the entire handler in a timeout (110 seconds - slightly less than mobile app's 120s timeout)
  const TIMEOUT_MS = 110000; // 110 seconds
  const timeoutPromise = new Promise<never>((_, reject) => {
    setTimeout(() => {
      reject(new Error('Recipe generation timed out after 110 seconds'));
    }, TIMEOUT_MS);
  });
  ...
  try {
      ...
      (async () => {
        try {
          ...
          if (error instanceof Error && error.message.includes('timed out')) {
            console.error('⏱️ Recipe generation timed out');
            return c.json({ 
              error: 'Request timeout',
              message: 'Recipe generation took too long. Please try again with a simpler request or a different dish.',
    ...
    if (error instanceof Error && error.message.includes('timed out')) {
      console.error('⏱️ Recipe generation timed out at handler level');
      return c.json({ 
        error: 'Request timeout',
        message: 'Recipe generation took too long. Please try again with a simpler request or a different dish.',
        details: error.message
    ...
    // Re-throw other errors to be handled by outer catch if needed
